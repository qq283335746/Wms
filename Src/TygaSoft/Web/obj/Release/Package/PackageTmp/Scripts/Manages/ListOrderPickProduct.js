var ListOrderPickProduct = {
    ...
    InitData: function () {
        this.InitForm();
        ListOrderPickProduct.LoadDg(1, 10);
    },
    ...
    InitForm: function () {
        ...
        pager.pagination({
            onSelectPage: function (pageNumber, pageSize) {
                ListOrderPickProduct.LoadDg(pageNumber, pageSize);
            }
        });
    },
    SelectRow: null,
    LoadDg: function (pageIndex, pageSize) {
        ...
        $.ajax({
            url: "/wms/Services/WmsService.svc/GetOrderPickProductList",
            type: "post",
            data: sData,
    ...
    FBtn: function (value, row, index) {
        ...
        if (qty > 0) {
            var qStr = 'keyName=OrderPickProduct&orderPickId=' + row.OrderPickId + '&orderId=' + row.OrderId + '&productId=' + row.ProductId + '&customerId=' + row.CustomerId + '&qty=' + qty;
            return '<a class="abtn" code="' + qStr + '" onclick="DlgStockLocationProduct.DlgOrderPickProduct(this)">操作</a>';
        }
        return '已完成';
    },
    ...
    OnDel: function () {
        ...
        $.messager.confirm('温馨提醒', '确定要删除吗？', function (r) {
            if (r) {
                $.ajax({
                    url: "/wms/Services/WmsService.svc/DeleteOrderPickProduct",
                    type: "post",
                    ...
                    success: function (result) {
                        ...
                        setTimeout(function () {
                            ListOrderPickProduct.LoadDg(1, 50, orderId);
                        }, 1000)
                    }
                });
            }
    ...
    UpdateRow: function (Id, orderId, productId, customerId) {
        var dg = $('#dgOrderProduct');

        var postData = { "ReqName": "GetOrderPickProductInfo", "Id": "" + Id + "", "OrderId": "" + orderId + "", "ProductId": "" + productId + "", "CustomerId": "" + customerId + "" };
        Common.AjaxPost("/wms/h/users.html", postData, function (result) {
            //console.log('GetOrderPickProductInfo--result--' + JSON.stringify(result));
    ...
    OnSearch: function () {
        ListOrderPickProduct.GetList(1, 10);
    }
}